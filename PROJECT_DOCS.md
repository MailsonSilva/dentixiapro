...
4.  **Vídeo de Boas-Vindas Vertical (Admin & Página Inicial):** Configurado via Painel Admin (`system_settings.welcome_video_url`). Na página inicial (`/`), é exibido em modal vertical 9:16 (80vh) se o campo `usuarios.check_video` for `false`. Ao marcar a opção "Já assisti a este vídeo", a action `setCheckVideoAction(true)` persiste no banco de dados e o modal não é reutilizado em logins futuros.
